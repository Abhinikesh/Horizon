...
/* ── Lightweight animated app mockup ── */
function AppMockup() {
        ...
        <div className="p-4 space-y-3">
            ...
            <div className="flex-1 flex items-end gap-0.5 h-7">
              {Array.from({ length: 30 }).map((_, i) => (
                <div key={i} className="flex-1 rounded-sm"
                  style={{ height: `${40 + Math.sin(i * 0.9) * 30}%`, background: i < 15 ? '#2563eb' : '#e5e7eb' }} />
              ))}
            </div>
          ...
          <div className="flex gap-2">
            {['Taj Mahal, Agra', 'AI Narrated', 'EN · HI · FR'].map(tag => (
              <span key={tag} className="px-2.5 py-1 text-[11px] font-medium rounded bg-gray-100 text-gray-600">{tag}</span>
            ))}
          </div>
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button onClick={onDemoOpen} className="btn-outline text-sm px-6 py-3">
                <Play size={15} />
                Watch Demo
              </button>
            </div>
